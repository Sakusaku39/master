            ...
            for src in chunk.srcs:  #係り先の番号のリストを回す
                for morph_s in data[i][src].morphs:
                    if morph_s.pos == '助詞': #助詞の入っている文節ならば
                        '''
                        source_postpositional.append(morph_s.surface)
                        source_phrase.append(''.join([morph_s.surface if morph_s.pos != '記号' else '' for morph_s in data[i][src].morphs]))
                        '''
                        phrase = ''.join([morph_s.surface if morph_s.pos != '記号' else '' for morph_s in data[i][src].morphs])
                        #sources[morph_s.surface] = phrase
                        source = {morph_s.surface:phrase}
                        ...
                        """
            '''
            source_postpositional.sort()
            source_phrase.sort()
            source_postpositional = ' '.join(source_postpositional)
            source_phrase = ' '.join(source_phrase)
            '''
            sources = sorted(sources.items())
            #key = ' '.join(sources.keys())
            #value = ' '.join(sources.values())
            #if sources != '':
                #print(chunk.srcs)
                #print('{}\t{}\t{}'.format(destination, key, value))
